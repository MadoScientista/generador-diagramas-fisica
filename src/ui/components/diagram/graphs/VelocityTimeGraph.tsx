...
interface VelocityTimeGraphProps {
  data: GraphData;
}
...
export function VelocityTimeGraph({ data }: VelocityTimeGraphProps) {
  ...
  const vMax = Math.max(0, ...vValues);
  const vRange = vMax - vMin || 1;
  const vPadding = vRange * 0.15;
  ...
  const linePoints = points.map((p) => `${toX(p.t).toFixed(2)},${toY(p.v).toFixed(2)}`).join(' ');

  const axisY0 = toY(0);
  const axisXEnd = toX(tMax);

  const timeTicks = computeNiceTicks(0, tMax).map((t) => ({ t, x: toX(t) }));
  const vTicks = computeNiceTicks(yMin, yMax).map((v) => ({ v, y: toY(v) }));

  const tauElements =
    tauStar !== null
      ? (() => {
          const vAtTau = points[0].v + points[0].a * tauStar;
          return `<line x1="${toX(tauStar)}" y1="${axisY0}" x2="${toX(tauStar)}" y2="${toY(vAtTau)}" stroke="${TAU_COLOR}" stroke-width="1" stroke-dasharray="4,3"/>
         <circle cx="${toX(tauStar)}" cy="${toY(vAtTau)}" r="4" fill="${TAU_COLOR}"/>
         <text x="${toX(tauStar) + 8}" y="${toY(vAtTau) - 8}" font-size="12" fill="${TAU_COLOR}" font-weight="500">τ* = ${formatValue(tauStar)} s</text>`;
        })()
      : '';
  ...
  ${vTicks.map((tick) => `<line x1="${MARGIN.left - 3}" y1="${tick.y}" x2="${MARGIN.left + 3}" y2="${tick.y}" stroke="${AXIS_COLOR}" stroke-width="1"/><text x="${MARGIN.left - 8}" y="${tick.y + 4}" text-anchor="end" font-size="11" fill="${AXIS_COLOR}">${formatValue(tick.v)}</text>`).join('\n  ')}
  <text x="${WIDTH / 2}" y="${HEIGHT - 8}" text-anchor="middle" font-size="13" fill="#000">Tiempo (s)</text>
  <text x="14" y="${HEIGHT / 2}" text-anchor="middle" font-size="13" fill="#000" transform="rotate(-90, 14, ${HEIGHT / 2})">Velocidad (m/s)</text>
